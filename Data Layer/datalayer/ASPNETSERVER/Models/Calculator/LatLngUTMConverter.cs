...
namespace ASPNETSERVER.Models.Calculator
{
    ...
    public class LatLngUTMConverter
    {
        ...
        public float[,] ArrResult(float lat, float longt)
        {
            //ask user for input
            LatLngUTMConverter converter = new LatLngUTMConverter(null);
           /* System.Console.WriteLine("Provide Latitude: ");
            double a = Convert.ToDouble(Console.ReadLine());
            System.Console.WriteLine("Provide Longitude: ");
            double b = Convert.ToDouble(Console.ReadLine());*/
            //convert lat/long to UTM
            UTMResult utm = converter.ConvertLatLngToUtm(lat, longt);
            Console.WriteLine(utm);
            //convert UTM to lat/long
            LatLng latlng = converter.convertUtmToLatLng(utm.Easting, utm.Northing, utm.ZoneNumber, utm.ZoneLetter);
            Console.WriteLine("Lat: " + latlng.Lat + ", Long: " + latlng.Lng);
         
            //insert data into array- array contains, long/lat, population density and confidence.
            var iRows = 10000;
            var iCols = 4;
            float[,] dataG = new float[iRows, iCols];
            int i;
            int j;
            string wtf = "";
            ...
            for (i = 0; i < iRows; i++)
            {
                ...
                if (i % 100 > 0)
                {
                    for (j = 0; j < iCols; j++)
                    {
                        ...
                        {
                            case 0:
                                dataG[i, j] = (float)(dataG[i - 1, 0] + 0.00002);
                                break;
                            case 1:
                                dataG[i, j] = (float)(dataG[i - 1, 1] + 0.001152);
                                break;
                            case 2:
                ...
                else
                {
                    for (j = 0; j < iCols; j++)
                    {
                        if (i == 0 && j == 0)
                        {
                            dataG[i, j] = (float)(latlng.Lat - ((0.00002 * 50) + (0.00090 * 50)));
                        }
                        else if (i == 0 && j == 1)
                        {
                            dataG[i, j] = (float)(latlng.Lng - ((0.001152 * 50) + (0.000026 * 50)));
                        ...
                        else
                        {
                            ...
                            {
                                case 0:
                                    dataG[i, j] = (float)(dataG[i - 100, 0] + 0.0009);
                                    break;
                                case 1:
                                    dataG[i, j] = (float)(dataG[i - 100, 1] + 0.000026);
                                    break;
                                case 2:
                                    dataG[i, j] = rnd.Next(1, 1000);
                                    break;
                                case 3:
                                    dataG[i, j] = rnd.Next(1, 100);
                                    break;
                            }
                        }
                    }
        ...
        public UTMResult ConvertLatLngToUtm(double latitude, double longitude)
        {
            ...
            //we NewEastinge Easting Northing value to the bottom value -00 
            var HMEasting = (int)UTMEasting - (int)UTMEasting % 100;
            var HMNorthing = (int)UTMNorthing - (int)UTMNorthing % 100;
            if ((int)UTMEasting % 100 > 50 && (int)UTMNorthing % 100 > 50)
            {
                HMEasting += 100;
                HMNorthing += 100;
            }
            else if ((int)UTMEasting % 100 > 50 && (int)UTMNorthing % 100 < 50)
            {
                HMEasting += 100;
            ...
            return new UTMResult { Easting = HMEasting, Northing = HMNorthing, ZoneNumber = ZoneNumber, ZoneLetter = UTMZone };
        }
